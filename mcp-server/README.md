...
Upload a **clean** directory containing only `main.py`, `requirements.txt`, and `Procfile`.
If you `railway up` from inside the orcavault Pages git root, Railpack may detect the site `index.html` and serve Caddy static instead of Python.

Live SSE: `https://orcavault-mcp-production.up.railway.app/sse`
